...
function dfsIterator( stateObj, callback, argUnknown, prevKeypath ) {
  rootKeypath = rootKeypath || '';
  
  if( !isComplexType( stateObj ) ) throw new TypeError( 'stateObj' );
  
  let stackKeys = _.keys( stateObj ).sort().reverse();
  
  while( stackKeys.lengh > 0 ) {
    let propName = stackKeys.peek();
    if( !isComplexType( stateObj[propName] )) {
      stackKeys.pop();
      // callback if forward
    }
    else {
      stateObj = stateObj[propName];
    }
  }
  ...
  keys.forEach(( key ) => {
    ...
    callback( key, keypath, value, stateObj );
...
function bfsIterator( obj, callback ) {
  ...
  _.keys( ctx ).sort().forEach( key => queue.push({ key: key, keypath: key, value: ctx[ key ], ctx: ctx }));
  ...
  while( queue.length > 0 ) {
    ...
    if( isComplexType( item.value )) {
      ...
      _.keys( ctx ).sort().forEach( key => queue.push({ key: key, keypath: item.keypath + '.' + key, value: ctx[ key ], ctx: ctx }));
    } 
  }
};

function bfsIterator2( obj, callback ) {
  ...
  _.keys( ctx ).sort().forEach( key => queue.push({ key: key, keypath: key, value: ctx[ key ], ctx: ctx, level: 0 }));
  ...
  while( queue.length > 0 ) {
    ...
    if( isComplexType( item.value )) {
      ...
      _.keys( ctx ).sort().forEach( key => queue.push({ key: key, keypath: item.keypath + '.' + key, value: ctx[ key ], ctx: ctx, level: item.level + 1 }));
    }
  }
